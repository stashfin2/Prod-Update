...
const getAndInsertInstallmentPif = async (data) => {
    try {
...
const deleteInstallmentsFromProd = async (loanIdString) => {
    try {
        const deleteDataFromProd = await mysql.query(`DELETE FROM installment_pif WHERE loan_id in (${loanIdString})`, [], 'prod');
...
getAndInsertInstallmentPif(workerData);
