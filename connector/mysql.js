  ...
  getConnections: async function (location) {
    return new Promise(async (resolve, reject) => {
      try {
        let getConnection =
          location === "prod"
            ? util.promisify(prodDbPool.getConnection).bind(prodDbPool)
            : util.promisify(loanTapeDbPool.getConnection).bind(loanTapeDbPool);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const beginTransaction = util
          .promisify(conn.beginTransaction)
          .bind(conn);
      ...
      try {
        conn =
          Object.keys(conn).length === 0
            ? await this.getConnections(location)
            : conn;
