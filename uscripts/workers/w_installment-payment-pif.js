...
if (isMainThread) {
  ...
  const processJob = async () => {
    try {
      ...
      const [totalCount] = await mysql.query(
        "SELECT COUNT(*) as count FROM paypif WHERE is_done = 0"
      );
  ...
  const processWorker = async () => {
    ...
    try {
      const rows = await mysql.query(
        "SELECT * FROM paypif WHERE is_done = 0 LIMIT ? OFFSET ?",
        [batchSize, offset]
      );
  ...
  const processChunk = async (data) => {
    ...
    try {
      ...
      for (let loanId in paymentRecords) {
        if (paymentRecords[loanId].length > 0) {
          for (let record of paymentRecords[loanId]) {
            let [updatedInstallmentPifArray, insPif, bucketAmountpif] =
              generatePayments(
                record["amt_payment"],
                record,
                groupedPif[loanId],
      ...
      const paymentsArray = createPaymentInst(
        installmentPaymentPifInsertionArray
      );
      ...
      const [del, result, upd] = await Promise.all([
        ...
        await mysql.query(
          `update paypif set is_done = 1 where ${whereClauses}`,
          [],
          "loan-tape"
        ),
      ]);

      logger.info("updated installment_payment_pif for this batch");
    } catch (error) {
      //await conn.rollback();
  ...
  const generatePayments = (
    ...
    try {
      ...
      if (col === "col") {
        logger.error(
          "Error while generating payments for installment_payment_pif table for colenders>>>>",
          error
        );
      } else {
        logger.error(
          "Error while generating payments for installment_payment_fip table fotcutomer facing>>>>",
          error
        );
      }
      logger.error(
        `Error occured while generating payments for installment_payment_pif table for array:`,
        error
      );
      throw error;
    }
  ...
  const sortByInstDateAndNumber = (arr) => {
    try {
      ...
      logger.error(
        `Error while sorting installments by number for installment_payment_pif table :`,
        error
      );
  ...
  const calculatePricipal = (principal, payment, bucketAmount, table) => {
    try {
      ...
      logger.error(
        `Error while calculating the pricipal payments in installment_payment_pif table:`,
        error
      );
      logger.error("Creds>>>>>>>>>>>>>>>>>>>>>>>>>>:");
  ...
  const calculateInterest = (principal, payment, bucketAmount, table) => {
    try {
      ...
      logger.error(
        `Error while calculating the INTEREST payments in installment_payment_pif table:`,
        error
      );
      logger.error("Creds>>>>>>>>>>>>>>>>>>>>>>>>>>:");
      logger.error("principal>>>>", JSON.stringify(principal));
  ...
  const insertPaymentsArray = async (paymentsArray, loanIdString) => {
    try {
      ...
      logger.error(
        `Error while inserting in installment_payment_pif table for loanIds (${loanIdString})`
      );
  ...
  const deleteInstallmentsFromProd = (loanIdString, whereClauses) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const deleteDataFromProd = `DELETE FROM installment_payment_pif WHERE ${whereClauses} and code_payment_type in (2,3)`;
  ...
  const getInstallments = (loanIdString) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const prodInstallments = await mysql.query(
          `Select * from installment_pif where loan_id in (${loanIdString}) order by inst_number asc`,
          [],
          "prod"
        );
