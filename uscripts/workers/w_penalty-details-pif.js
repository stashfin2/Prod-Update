...
if (isMainThread) {
  ...
  const processJob = async () => {
    try {
      ...
      const [totalCount] = await mysql.query(
        "SELECT COUNT(*) as count FROM pdpif_t WHERE is_done = 0"
      );
  ...
  const processWorker = async () => {
    ...
    try {
      const rows = await mysql.query(
        "SELECT * FROM pdpif_t WHERE is_done = 0 LIMIT ? OFFSET ?",
        [batchSize, offset]
      );
  ...
  const processChunk = async (data) => {
    ...
    try {
      const [loanTapePenalties, deleteOp] = await Promise.all([
        ...
        deleteDataFromPenaltyDetailsPif(loanIds, whereClauses),
      ]);

      const insertOp = await insertPenaltiesPif(loanTapePenalties, loanIds);
      const lpfp = await mysql.query(
        `update pdpif_t set is_done = 1 where ${whereClausesForUpdate}`,
        [],
        "loan-tape"
      );

      logger.info(
        "All operations for data tranasfer for penalty_details_pif from loan-tape to prod has been done"
      );
    } catch (error) {
      //await conn.rollback();
  ...
  const insertPenaltiesPif = async (data, loanIds) => {
    try {
      logger.info(
      ...
      logger.info(
        `Successfully inserted ${result.affectedRows} penalties inside the penalty_details_pif table`
      );
      return result;
    } catch (error) {
      logger.error(
  ...
  const deleteDataFromPenaltyDetailsPif = async (loanIds, whereClauses) => {
    try {
      logger.info(
        `Deleting penalties from penalty_details table for loan_ids and is_paid = 0: ${loanIds}`
      );
      ...
      logger.info(
        `Deleted ${result.affectedRows} rows from penalty_details_pif table`
      );
      return result;
    } catch (error) {
