    ...
    async transform(batch, encoding, done) {
      try {
        let promises = batch.map((row) =>
          mysql.query(
            "update installment_pif_cl set days_past_due = ?, emi_status_id = ? where id = ?",
        ...
        logger.error("Error while updating installment_pif_cl:", error);
...
const processJob = async () => {
  try {
    ...
    const dataStream = conn
      .query("select * from installment_pif_cl where is_delete = 0 and customer_facing = 1 and inst_status = 1 and emi_status_id in (1,3) and inst_date < date(now()) limit 1000", [])
      .stream();
